...
# (?:(?:(?P=series)(?P=ordinal))|
#    (:?Int/(?P=series_internal))|
#    (?P=precursor)|
#    (:?I(?P=immonium)(?:(?P=immonium_modification)|[A-Z])?)|
#    (?P=reporter_mass)|
#    (?:_(?P=external_ion))
# )
...
class SPTXTAnnotationStringParser(annotation.AnnotationStringParser):
    ...
    def _dispatch_immonium(self, data, adducts, charge, isotope, neutral_losses, analyte_reference, mass_error, **kwargs):
        ...
        return super(SPTXTAnnotationStringParser, self)._dispatch_immonium(
            data, adducts, charge, isotope, neutral_losses, analyte_reference, mass_error, **kwargs)
...
parse_annotation = SPTXTAnnotationStringParser(annotation_pattern)


class SPTXTSpectralLibrary(_PlainTextSpectralLibraryBackendBase):
    file_format = "sptxt"
    format_name = "sptxt"
